...
try:
    ...
    regex_lst = [reg_gatineau, reg_GMA, reg_laurent, reg_drum, reg_maur, reg_town, reg_GQA, reg_abit, reg_temis,
                 reg_LSJ,
    ...
    weekdayMatcher = re.compile(
        "(TONIGHT|TODAY|MONDAY|TUESDAY|WEDNESDAY|THURSDAY|FRIDAY|SATURDAY|SUNDAY)\.\.AIR QUALITY ")
    ...
    lstofAmendment = []
    for files in os.listdir(path + "tempdir"):
        AM_PM = files[:2]
        f = open(path + "tempdir/" + files).read()
    ...
    for files, amd in zip(os.listdir(path + "tempdir"), lstofAmendment):
        f = open(path + "tempdir/" + files).read()
        k = k + 1
        # progress(k, prog, '')
        for y in start.finditer(f):
            yearstr = f[y.start():y.end() + 410]
            ...
            for element in reg.finditer(f):
                ...
                for day, ind in zip(weekdayMatcher.finditer(textTomatch), indice.finditer(textTomatch)):
                    ...
                    for dayWrite in daytoWrite.finditer(day.group()):
                        ...
                        date = fixedTime.split("-")
                        yearHour = fixdate(date[1] + date[2])
                        Paire = pairValuefinder(dayWrite.group(), fixedTime[:2])
                        Indince = ind.group()
    ...
    sender = 'InfoSmog-hcron'
    receivers = ['ding.ma@canada.ca']

    message = """
    An error has occured while processing InfoSmog bulletins.
    """
